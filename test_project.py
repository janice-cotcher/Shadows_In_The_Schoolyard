...
def test_check_room_action(sample_room, sample_inventory):
    result = check_room_action(
        f"Question {sample_room.person}", sample_room, [sample_inventory], []
    )
    assert result == False
...
def test_handle_retirement_manor():
    ...
    with patch("builtins.input", return_value="Zachariah Dowey"):
        attempts_trinity, salt_bool, blessed_salt_bool, cache, new_room = (
            handle_retirement_manor(room, "Question Phil Callahan", 0, False, False, [])
        )
...
def test_handle_retirement_herring():
    ...
    with patch("builtins.input", return_value="Sonny O'Sullivan"):
        attempts_carehome = handle_retirement_herring(
            room, "Question Laurence Tureaud", 0
        )
    ...
    with patch("builtins.input", return_value="Wrong Name"):
        attempts_carehome = handle_retirement_herring(
            room, "Question Laurence Tureaud", 1
        )
    ...
    with patch("builtins.input", return_value="Wrong Name"):
        attempts_carehome = handle_retirement_herring(
            room, "Question Laurence Tureaud", 2
        )
...
def test_get_rooms_for_location():
    location1 = Location(
        "Location1", [Room("Location1", "Room1"), Room("Location1", "Room2")]
    )
...
Chant: Away Tsathoggua!"""
    ...
    with patch(
        "builtins.input",
        side_effect=[line for line in test_input.split("\n")] + [EOFError],
    ):
